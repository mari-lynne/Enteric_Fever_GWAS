...
load(
  "/home/mari/RNA/Filter2_VAST_STAR_HTSeq_gene_meta_autosomes_mismatch_corrected_demo_minus_rRNA_globins_autosomes_2021-04-27.R"
)
...
```{r Filter time points, include = FALSE}
...
#Keep control time points, but remove pre-chall timepoints in order to avoid linear dependencies issues later #Will probably have an issue with TD time point, that might need to be a seperate cohort
...
```{r TPM}
data <- data2
#save.image(file = "vast_filtered_raw.RData")
...
tpm <- tpm3(data$counts, data$genes)
exprs <- as.data.frame(t(tpm))
exprs$row_names <- rownames(exprs)

pheno <- data$samples
pheno_exprs <- left_join(pheno, exprs, by = "row_names")
write.csv(pheno_exprs, file = "vast_tpm.csv") # also batch adjusted in combat_rnaseq

# Raw counts table
exprs <- as.data.frame(t(data$counts))
exprs$row_names <- rownames(exprs)
pheno <- data$meta_data
pheno_exprs <- left_join(pheno, exprs, by = "row_names")
write.csv(pheno_exprs, file = "vast_tyg_comb_raw.csv")
```
...
data$samples$time_point3 <- as.factor(data$samples$time_point3)
data$samples$diagnosis <- as.factor(data$samples$diagnosis)

# merged design
data$samples$group <- str_c(data$samples$diagnosis, data$samples$time_point3, sep = "_")
...
idx <- (data$samples$diagnosis != "UNKNOWN - Not Challenged")
sub <- data[,idx]
dim(sub)
data <- sub
...
#samples have data for the design categories, however are missing others
...
colnames(design) <- gsub("D0\\+", "D0.", colnames(design))
colnames(design) <- gsub("Sex", "", colnames(design))
colnames(design) <- gsub("sequence_", "", colnames(design))
