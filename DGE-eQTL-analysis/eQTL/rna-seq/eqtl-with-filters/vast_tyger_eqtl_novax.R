...
#Load
data <- readRDS(file = "~/RNA/oct/vast_tyger/VT_novax.Rds") # counts are TPM
...
time <- c("TD")
var <- c("_gwas_")
deg <- read.csv(file = "~/RNA/oct/V7_V0_top.csv")
...
idx <- (data$samples$time_point == "TD")
sub <- data[,idx]
dim(sub)
...
deg <- deg2 %>% dplyr::filter(adj_p_val <= 0.05 & log_fc > 0.2 | log_fc < -0.2)
...
covar <- dplyr::select(covar, IID, sex, age, bmi, packs_per_week) # vaccine)
...
write.csv(eqtl_sig,
          file = paste0(out_dir, time_point, var,"sig_eqtl_immune_dge_novax.csv"),
          row.names = FALSE)

write.csv(eqtls,
          file = paste0(out_dir, time_point, var,"all_eqtl_immune_dge_novax.csv"),
          row.names = FALSE)

write.csv(eqtls,
          file = paste0(out_dir, time_point, var,"cis_immune_dge_novax.csv"),
          row.names = FALSE)
