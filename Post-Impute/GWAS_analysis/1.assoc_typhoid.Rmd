...
knitr::opts_knit$set(root.dir = '~/GWAS_22/gwas_final/merge/enteric/assoc')
...
gwas <- fread("enteric.Diagnosed.glm.logistic.hybrid")

gwas <-
  filter(gwas, TEST == "ADD") 

# Rename cols - Logistic
...
# Inspect top snps: ------------------------------------------------------------
tophits <- filter(gwas, P < 1e-5)

write.table(tophits, file = "tophits_enteric.txt", quote = F, row.names= F, sep = "\t")

write.table(gwas, file = "gwas_enteric.txt", quote = F, row.names= F, sep = "\t")
# delete og assoc output as very large

# QQ plot
plot_dir <- c("~/GWAS_22/gwas_final/merge/enteric/assoc/plots/")
png(file = (paste(plot_dir,"enteric_qq.png", sep = "")), width = 7, height = 7, units = "in", res = 300)
qq(gwas$P)
dev.off()
```
...
```{r custom-manhattan-setup}
gwas <- fread("gwas_enteric.txt")
...
# Basic Plot ----------------------------------------------------
plot_dir <- c("~/GWAS_22/gwas_final/merge/enteric/assoc/plots/")
png(file = (paste(plot_dir,"enteric-man.png", sep = "")), width = 9, height = 6, units = "in", res = 300)
ggplot(gwas_data,
       aes(
         x = bp_cum,
         y = -log10(p),
