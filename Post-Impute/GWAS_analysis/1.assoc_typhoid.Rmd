...
Ensembl coords +/- 10 KB
Starting from FcGR2a Chromosome 1: 161,505,430
To FcGR2b Chromosome 1: 161,678,654 
...
Ensembl coords GRCh38.p13:
19: 54,874,235-54,891,420
...
--out fcar_assoc
 ...
 # I.e is there a more sig snp in a nearby LD gene that is responsible for the association
```
### VAST antibody titre GWAS
```{bash GWAS VAST}
plink2 --bfile ~/GWAS_22/gwas_final/merge/typhoid/QC/typhoid2.IBD \
--maf 0.1 \
--pheno ~/GWAS_22/gwas_final/meta/vast_pheno.txt \
--pheno-name adnob \
--covar ~/GWAS_22/gwas_final/meta/typhoid_pcacovar.txt \
--covar-name sex, chall_vax, PC1, PC2, PC3, PC4, PC5 \
--covar-variance-standardize \
...
# Typhoid Assoc -----------------------------------------------------------------
gwas <- fread("vast.adnob.glm.linear")
...
write.table(gwas, file = "vast_adnob.txt", quote = F, row.names= F, sep = "\t")
# delete og assoc output as very large

#gwas <- fread("gwas_typhoid.txt")

# Inspect top snps: ------------------------------------------------------------
tophits <- filter(gwas, P < 1e-5)

write.table(tophits, file = "tophits_vast_adnob_igg.txt", quote = F, row.names= F, sep = "\t")

#### Manhattan Plots
plot_dir <- c("~/GWAS_22/gwas_final/merge/typhoid/assoc/plots/")

#qq_plot
png(file = (paste(plot_dir,"adnob_qq.png", sep = "")), width = 5, height = 5, units = "in", res = 300)
qq(gwas_data$p)
dev.off()
```
...
png(file = (paste(plot_dir,"adnob_man.png", sep = "")), width = 9, height = 6, units = "in", res = 300)
ggplot(gwas_data,
       aes(
         x = bp_cum,
         y = -log10(p),
