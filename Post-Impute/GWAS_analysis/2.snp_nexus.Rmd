...
tophits <- fread("tophits_typhoid.txt")
...
write.table(nexus, "typhoid_nexus.txt", row.names = F, col.names = F, quote = F, sep = " ")
...
```{r get-nexusdata}
# Read in data
temp = list.files(pattern = "*uk.txt")
...
tophits <- select(tophits, -c(TEST,OBS_CT))
...
gwas_data <- clean_names(fread("gwas_typhoid.txt"))
...
png(file = (paste(plot_dir,"top-genes-man.png", sep = "")), width = 10, height = 6, units = "in", res = 300)
ggplot(gwas_data,
       aes(
         x = bp_cum,
         y = -log10(p),
  ...
  geom_label_repel(
    ...
    size = 2.5, color = "#D28423", nudge_y = 0.5, nudge_x = 0.1
  ) +
  geom_label_repel(
