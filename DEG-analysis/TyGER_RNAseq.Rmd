...
plot_dir <- c("~/RNA/oct/plots/") #where to save output plots
...
# fully processed vast data
load(file = "VAST_deg_nov10.RData")
...
table(data$metadata$time)
#Rename variables
names(data$metadata)[names(data$metadata) == "days_since_challenge"] <-
  "time"

samples <- cbind(data$samples, data$metadata)
genes <- data$genes
counts <- data$counts
...
samples$study_arm <- as.character(samples$study_arm)
...
idx <- (data$samples$time_point %in% c("Baseline","D0.12h", "D7") & data$samples$diagnosis %in% c("TD", "nTD"))
...
cpm <- cpm(data$counts)
lcpm <- cpm(data$counts, log=TRUE) #Used for exploratory plots/checks
...
#save.image(file = "TyGER_norm.RData")
...
png(paste(plot_dir,"filter.png",sep =""))
par(mfrow=c(1,2))
...
-   You can also test for two factors together using interaction terms, e.g\
    `group = interaction(data$metadata$sequence_pool,data$metadata$TimePoint)`

```{r MDS plot}
#Sequencing lane check
var <- c("vast")
library(ggplot2)
group = as.factor(data$samples$batch)
...
#Plot MDS
png(paste(plot_dir, "batch.png", sep = ""))
plotMDS(lcpm, labels = group, col = visit)
title(main = "Visit MDS")
dev.off()
```
...
-   e.g `limma::removeBatchEffect(cpm, data$metadata$sequence_pool)`
...
# merged design
data$samples$group <- str_c(data$samples$diagnosis, data$samples$time_point, sep = "_")
table(data$samples$group)
data$samples$group <- as.factor(data$samples$group)

design <-
  model.matrix(~0 +group + study_arm + age_at_do +sex +sequence_pool, data = data$samples)

#Tidy new design colnames
colnames(design) <- gsub("study_arm", "", colnames(design))
colnames(design) <- gsub("diagnosis", "", colnames(design))
colnames(design) <- gsub("time_point", "", colnames(design))
colnames(design) <- gsub("sequence_pool", "batch_", colnames(design))
...
pdf(paste(plot_dir, "voom_1.pdf", sep = ""))
...
save.image(file = "VAST_deg_nov10.RData")
...
contrasts <- makeContrasts(
  nTD_D0.12h_vs_D0 = nTD_D0.12h - nTD_Baseline,
  TD_D0.12h_vs_D0 = TD_D0.12h - TD_Baseline,
  TD_v_nTD_D0 = (TD_D0.12h - TD_Baseline) - (nTD_D0.12h - nTD_Baseline),
  nTD_D7_vs_D0 = nTD_D7 - nTD_Baseline,
  TD_D7_vs_D0 = TD_D7 - TD_Baseline,
  TD_v_nTD_D7 = (TD_D7 - TD_Baseline) - (nTD_D7 - nTD_Baseline),
  levels = colnames(design)
) 

fit2 <- contrasts.fit(fit, contrasts)
fit2 <- eBayes(fit2)
h12_D0_nTD <- topTable(fit2, coef = 1, number = Inf)
h12_D0_TD <- topTable(fit2, coef = 2, number = Inf)
h12_TDnTD <- topTable(fit2, coef = 3, number = Inf)
D7_TDnTD <- topTable(fit2, coef = 6, number = Inf)
TD_vs_D0 <- topTable(fit2, coef = 5, number = Inf)
D7_vs_D0 <- topTable(fit2, coef = 4, number = Inf) #nTD

# 3rd and six coef = mean diffs between diagnosis groups
...
plot_title <- c("TD vs nTD - 24h Post-vaccination")

# Add gene info if tyger
#deg <- D7_TDnTD
...
# UPDATE
# Modify gene names
# "GP9", "ITGB3" # "THBS1 # "MMRN1" # "MPL" # "GP1BA" # "CD9" # ,"TBCA"

qtls <- c("F2R", "F2RL1", "ANGPT1","PTGS1", "AGGF1", "TRIM69", "B2M")
...
p <- plot_vol(deg,
           lab = 'ns',
           genes = qtls,
           title = "12h post-challenge - nTD")
p

#Save plots ####
png(file = paste(plot_dir,"12h_nTD.png", sep = ""))
p
dev.off()

```
Calculate FC by participant
```{r FC}

load(file = "~/RNA/oct/VAST_deg_nov10.RData")

# subset genes and baseline participants

idx <- data$genes$gene_name %in% c("F2R")
sub <- data[idx,]
dim(sub)

idx <- (sub$samples$time_point == "Baseline")
base <- sub[,idx]
dim(base)

idx <- (sub$samples$time_point == "D7")
D7 <- sub[,idx]
dim(D7)

# make pheno exprs

exprs <- as.data.frame(t(base$counts))
  names(exprs) <- base$genes$gene_name
  pheno <- as.data.frame(base$samples)
  pheno_exprs <- bind_cols(pheno, exprs)
  
 exprs2 <- as.data.frame(t(D7$counts))
  names(exprs2) <- D7$genes$gene_name
  pheno2 <- as.data.frame(D7$samples)
  pheno_exprs2 <- bind_cols(pheno2, exprs2)
  
merge <- inner_join(pheno_exprs2, pheno_exprs, by = "lab_id") # y is baseline val
merge <- select(merge, lab_id, time_point3.x, time_point.x, study_arm.x, diagnosis.x,F2R.y, F2R.x)
merge <- merge %>%
  group_by(lab_id) %>%
  mutate(FC =
           (F2R.x - F2R.y)
...
merge %>% ggplot(aes(x = diagnosis.x, y = FC)) + geom_boxplot(outlier.shape = NA)


load(file = "~/GWAS_22/gwas_final/eQTL/chr5_platelets.RData")
test <- snp_pheno %>% select(lab_id, F2R)
merge2 <- inner_join(snp_pheno, merge, by = "lab_id")

merge %>% ggplot(aes(x = diagnosis.x, y = FC)) + geom_boxplot(outlier.shape = NA)

merge2 %>% filter(!is.na(Chr5_77685638_A_G)) %>%
    ggplot(aes(x = Chr5_77685638_A_G, y = FC, fill = Chr5_77685638_A_G)) +
    geom_boxplot(outlier.shape = NA) +
    scale_fill_manual(values = c("#A92D2F", "#CE7726", "#9B9759"))

merge2 %>% filter(!is.na(Chr5_77682126_C_T)) %>%
    ggplot(aes(x = Chr5_77682126_C_T, y = FC, fill = Chr5_77682126_C_T)) +
    geom_boxplot(outlier.shape = NA) +
    scale_fill_manual(values = c("#A92D2F", "#CE7726", "#9B9759"))

write.csv(merge2, file = "F2R_FC_VT.csv", row.names = FALSE)
