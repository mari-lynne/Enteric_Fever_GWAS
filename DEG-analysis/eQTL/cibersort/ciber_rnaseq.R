...
data$meta_data$arm_time <- str_c(data$meta_data$study_arm, data$meta_data$time, sep = "_")

# Get expression data
combat <- sva::ComBat_seq(counts=data$counts,
                          batch=data$meta_data$sequence_pool,
                          group=data$meta_data$arm_time)
...
tpm <- tpm3(combat, data$genes$transcript_length)
